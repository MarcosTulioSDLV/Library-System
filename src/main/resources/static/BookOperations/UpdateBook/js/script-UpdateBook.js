...
updateBookForm.addEventListener('submit',async (event) => {
    ...
    try {
        const updatedBook = await updateBook({'id':id,'isbn':isbnText.value,'name': nameText.value,'edition': editionText.value,'publicationYear':yearText.value});
...
backButton.addEventListener('click',()=> {
  window.location.href= "/BookOperations/BookOperations.html";//method 2 //window.history.back();
});
...
async function getBook(id){
    ...
    try {
      ...
      const data = await response.json();
      return data; // This will be a Book
    } catch (error) {
        console.error('There was a problem with the fetch operation:', error);
        throw error; // Rethrow the error for handling at the caller's level
    }
